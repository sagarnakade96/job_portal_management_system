...
## 🔧 Known Fixes (already applied to this version)

| Prompt | Issue | Fix Applied |
|---|---|---|
| PROMPT 2.2 | `motor==3.4.0` incompatible with `pymongo 4.9+` — throws `ImportError: cannot import name '_QUERY_OPTIONS'` | Upgraded to `motor==3.6.0` |
| PROMPT 2.5 | Jobs `main.py` missing CORS — browser dashboard shows `Failed to fetch` | Added `CORSMiddleware` with `allow_origins=["*"]` |
| PROMPT 3.1 | `cors` npm package not installed — candidates service blocked all browser requests | Added `cors` to `npm install` command |
| PROMPT 3.3 | `.gitkeep` in `src/routes/` caused agent to skip writing the file — container crashed with `Cannot find module './routes/candidates'` | Added pre-step to delete `.gitkeep` before writing the route file |
| PROMPT 3.4 | Candidates `index.js` missing `cors` middleware | Added `require('cors')` and `app.use(cors())` |
| PROMPT 4.5 | Applications `main.py` missing CORS — `OPTIONS` preflight returned `405` | Added `CORSMiddleware` with `allow_credentials=True` |
...
# ⚠️ motor==3.6.0 required — motor 3.4.0 is incompatible with pymongo 4.9+ and will throw
...
> **VERIFY:** `package.json` exists in `services/candidates/` with express + mongoose + node-fetch + cors.
...
npm init -y
npm install express mongoose dotenv node-fetch cors
...
> **YOU:** Before feeding this prompt, verify the target directory is clean: `ls ~/job-board/services/candidates/src/routes/` — if only `.gitkeep` exists, the agent may skip writing. Delete it first: `rm ~/job-board/services/candidates/src/routes/.gitkeep`
> **VERIFY:** `cat ~/job-board/services/candidates/src/routes/candidates.js` shows the full router file with 5 endpoints.
